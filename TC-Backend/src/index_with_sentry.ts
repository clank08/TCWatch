...
import { initializeSentry, sentryFastifyPlugin, captureException, flushSentry } from './utils/sentry';
initializeSentry();
...
const start = async () => {
  try {
    ...
    // Register Sentry plugin for error tracking
    ...
    captureException(err as Error, { startup: true });
    await flushSentry();
    process.exit(1);
  }
};
...
const gracefulShutdown = async (signal: string) => {
  ...
  try {
    ...
    captureException(err as Error, { shutdown: true });
    await flushSentry();
    process.exit(1);
  }
};
...
process.on('uncaughtException', async (err) => {
  ...
  captureException(err, { uncaught: true });
  await flushSentry();
  process.exit(1);
});
...
process.on('unhandledRejection', async (err) => {
  ...
  captureException(err as Error, { unhandled: true });
  await flushSentry();
  process.exit(1);
});
